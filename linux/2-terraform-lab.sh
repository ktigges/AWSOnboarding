...
find_ami() {
  ...
  if [[ -z "${AMI_ID:-}" ]]; then
    ...
    local count
    count=$(printf '%s\n' "$amis" | wc -w | tr -d ' ')
    [[ "$count" == "1" ]] || fail "Expected exactly one MDE golden AMI, found $count. Run ./linux/1-build-ami.sh or remove duplicates."
    AMI_ID=$amis
  fi
}
...
ensure_azure_auth() {
  ...
  [[ -n "${AZURE_TENANT_ID:-}" ]] || fail "AZURE_TENANT_ID is required for Intune and Entra cleanup"
}
...
case "$ACTION" in
  plan|apply)
    ...
    if [[ "$ACTION" == apply ]]; then
      ...
      printf 'Destroy with: ./linux/2-terraform-lab.sh destroy\n'
    else
  ...
  destroy)
    ...
    printf 'The Packer AMI and snapshot remain until you run: ./linux/3-deregister-ami.sh --confirm\n'
    ;;
esac
